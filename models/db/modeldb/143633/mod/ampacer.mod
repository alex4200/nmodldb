...
NEURON {
	POINT_PROCESS AMPAcer
	POINTER pre
	RANGE C, R, R0, R1, g, gmax, lastrelease, D, D0, D1
...
PARAMETER {
	...
	d = 1.0 				: 0.9 proportional factor for multiplying depression (D) upon arrival of presyn AP
	Dtau = 200 (ms)		: 250 time constant for decay of D back to 1
	Dinf = 1				: D always decays back to 1
...
ASSIGNED {
	...
	D				: depression factor
	D0				: facilitation factor at start of release
	D1				: facilitation factor at end of release
}
...
PROCEDURE release() { LOCAL q
	...
	D = Dinf - (Dinf - D0) * exptable (- (t - lastrelease) / Dtau)
